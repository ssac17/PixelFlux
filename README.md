...
> **JavaFX 기반의 이미지/동영상 배치 처리 도구**
>
> 드래그 앤 드롭으로 대량의 미디어 파일을 병렬 처리하여 빠르게 변환합니다.
> 포맷 변환, 해상도 조절, 품질 최적화 기능을 지원합니다.

<small>mac환경에서 실습하여 실제 프로그램은 올리지 못했습니다. 후추 window용으로 제작하여 첨부하겠습니다.</small>

---
## 💡 기획 배경 (Why PixelFlux?)

각종 정부 기관, 채용 사이트, 금융 플랫폼 등에 서류나 증빙자료를 제출할 때마다 마주치는 **엄격한 파일 용량 제한과 특정 확장자(JPG, PNG, PDF 등) 요구사항**에서 시작되었습니다.

기존 웹 기반 변환 도구들은 다음과 같은 치명적인 불편함이 있었습니다:
- **과금 및 횟수 제한**: 무료인 줄 알고 변환했으나 다운로드 단계에서 결제를 유도하거나 하루 변환 횟수를 엄격히 제한
- **보안 및 개인정보 유출 우려**: 주민등록등본, 통장 사본 등 민감한 개인 증빙서류를 신뢰하기 어려운 외부 웹 서버에 업로드해야 하는 위험
- **대량 처리의 불편함**: 여러 장의 서류를 한 번에 리사이즈하거나 포맷을 일괄 변경하기 어려움

이러한 비효율을 해결하고자, **외부 서버 통신 없이 로컬 환경에서 100% 안전하게 동작하는 무료 무제한 미디어 변환 도구**를 직접 개발하게 되었습니다.
